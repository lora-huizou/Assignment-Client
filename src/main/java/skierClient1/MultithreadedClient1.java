...
public class MultithreadedClient1 {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    for (int i = 0; i < INITIAL_THREADS; i++) {
      threads[i] = new Thread(new LiftRidePostWorker1(eventQueue, REQUESTS_PER_THREAD,requestCounter));
      threads[i].start();
    }
    ...
    for (int i = 0; i < additionalThreads; i++) {
      ...
      threads[i] = new Thread(new LiftRidePostWorker1(eventQueue, requestsForThread, requestCounter));
      threads[i].start();
